...
dist=np.array(
 [
    0.0006137973962876561,
    -0.008005966884760359,
    0.0012953684669048207,
    0.0006712604466038405,
    0.019456820140031063
 ]
)
dist = np.reshape(dist, (5,1))
print(dist)
...
points_3d = np.reshape(points_3d, (4,3,1))
MESH = np.array([
...
tree = ET.parse('ANNOTATIONS/6_2-experiment1_points.xml')
root = tree.getroot()
dist=None
#out = cv2.VideoWriter('test2.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 60, (1920, 1080))
...
for idx, c in enumerate(root[2:]):
    ...
    points = np.reshape(points, (4,2,1))
    ...
    im = cv2.imread(r'../DATA/6_2/GH010434/' +str(3369+idx).zfill(5) + '.jpg')     
    ...
    #out.write(os.path.join('temp', str(idx).zfill(4) + '.jpg'))
